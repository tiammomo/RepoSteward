...
协议职责、schema 版本和交付范围见[协议与兼容性索引](protocol-map.zh-CN.md)。

## 分包之后的架构边界

这一轮完成的是代码归类。`workflows/pipeline.py` 仍集中承担多种维护流程，
`storage/store.py` 仍包含大部分台账存取和迁移；`cli.py` 继续承担命令组装与分发。
`core/capabilities.py` 也会导入 MCP 工具 schema，所以 `core/` 不能被理解为已经完全
隔离适配器依赖的领域层。目录清晰不等于依赖解耦已经完成。

后续按实际修改需求逐步收敛：

1. 从编排模块提取发布、审阅、恢复等完整业务流程，保留同一授权、审计与幂等边界。
2. 按业务聚合整理存储访问，保持共享事务及迁移顺序，不为目录拆分而拆开原子事务。
3. CLI、HTTP、MCP、A2A 复用应用服务与契约；入口只处理参数、作用域与传输结果。

每次拆解由独立 Issue 定义可验证行为，先复用相关回归和安装验收，再逐步建立依赖约束。
不要只根据行数切文件，也不要为每个命令、类或数据表建立独立包。
...
FastAPI/React、持久队列、项目导入、工作区扫描和 A2A 已在对应职责包中交付。
后续扩展继续按独立 Issue 实现，并同步 import、测试 mock 字符串和构建脚本中的路径。
