...
外部尝试的结束由独立 `TaskLifecycle` 应用服务负责，`TaskLifecycleRecords` 封装
终态记录存取，复用同一个 Store 事务。结束结果和开发检查点分别保存；完成必须
关联精确原生合并证据，取消和接替保留处置原因。CLI 提供计划与执行入口，MCP 和
工作台复用共享任务查询，不各自推断完成。见[任务生命周期](task-lifecycle.zh-CN.md)。
